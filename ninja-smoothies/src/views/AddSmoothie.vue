	...
	<div class="add-smoothie container">
		...
		<form @submit.prevent="AddSmoothie">
			<div class="field title">
				...
				<input type="text" id="titleInput" name="title" v-model="title" />
			</div>
			<div class="field add-ingredient">
				<label for="add-ingredient">Add your ingredients</label>
				<input
					id="ingInput"
					type="text"
					autocomplete="off"
...
export default {
	...
	methods: {
		AddSmoothie() {
			...
			const collection = db.collection('smoothies')

			collection
				.get()
				.then(snapshot => {
					const slugsDB = []
					snapshot.forEach(document => slugsDB.push(document.data().slug))

					let suffix = ''
					let i = 2

					const condition = () =>
						slugsDB.find(item => item === newSlug + suffix)

					while (condition()) {
						suffix = '-' + i
						i++
					}
					this.slug = newSlug + suffix
				})
				.then(() =>
					collection
						.add({
